...
contract GOKURebaser {
     ...
     * @notice Sets the reserve contract
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 gokusSold, uint256 gokusFromReserves, uint256 gokusToReserves);
    /**
     * @notice Event emitted when pendingGov is changed
     */
    event NewPendingGov(address oldPendingGov, address newPendingGov);
    ...
    /// @notice Whether or not this token is first in uniswap GOKU<>Reserve pair
        ...
        public
    {
          ...
          (address token0, address token1) = sortTokens(gokuAddress_, reserveToken_);
          // used for interacting with uniswap
          if (token0 == gokuAddress_) {
              isToken0 = true;
          } else {
              isToken0 = false;
          }
          // uniswap GOKU<>Reserve pair
    ...
    */
        ...
        public
    {
        ...
        GOKUTokenInterface goku = GOKUTokenInterface(gokuAddress);
        if (positive) {
            require(goku.gokusScalingFactor().mul(uint256(10**18).add(indexDelta)).div(10**18) < goku.maxScalingFactor(), "new scaling factor will be too big");
        }
        uint256 currSupply = goku.totalSupply();
        uint256 mintAmount;
        ...
        // rebase
        uint256 supplyAfterRebase = goku.rebase(epoch, indexDelta, positive);
        assert(goku.gokusScalingFactor() <= goku.maxScalingFactor());
        ...
        public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            goku.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.gokusToUni) {
                // if the amount from reserves > gokusToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                goku.mint(uniswap_pair, uniVars.gokusToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            goku.mint(uniswap_pair, uniVars.gokusToUni);
        }
        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            goku.mint(reservesContract, uniVars.mintToReserves);
        }
        // transfer reserve token to reserves
        if (isToken0) {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
            emit TreasuryIncreased(amount1, uniVars.gokusToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        } else {
        ...
        internal
    {
        ...
        // check if protocol has excess goku in the reserve
        uint256 excess = goku.balanceOf(reservesContract);
        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);
        UniVars memory uniVars = UniVars({
          gokusToUni: tokens_to_max_slippage, // how many gokus uniswap needs
          amountFromReserves: excess, // how much of gokusToUni comes from reserves
          mintToReserves: 0 // how much gokus protocol mints to reserves
        });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.gokusToUni = mintAmount + excess;
                ...
                if (tokens_to_max_slippage > excess) {
                    ...
                    // swap up to slippage limit, taking entire goku reserves, and minting part of total
