 ...
 */
pragma solidity 0.5.16;
contract NFI is ERC20, ERC20Detailed {
    ...
    // Modifiers
    modifier onlyGov() {
        require(msg.sender == governance, "NFI-Token: You are not the governance");
        _;
    }
    constructor () public ERC20Detailed("ocb.finance", "NFI", 18, 1500000 * 10 ** 18) {
    ...
    function mint(address _account, uint256 _amount) external {
        require(minters[msg.sender], "NFI-Token: You are not the minter");
    ...
    function acceptGov()
        external {
        require(msg.sender == pendingGov, "NFI-Token: You are not the pending governance");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal {
        ...
        if (from == address(0)) { // When minting tokens
            require(totalSupply().add(amount) <= cap(), "NFI-Token: Capacity exceeded");
        }
    }
}